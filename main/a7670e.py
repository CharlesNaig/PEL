...
def find_usb_at_port():
    ...
    if simcom_ports:
        log.info(f"[A7670E] Found SIMCOM USB ports: {simcom_ports}")
        # AT port is typically the 3rd one (index 2)
        if len(simcom_ports) >= 3:
            at_candidate = simcom_ports[2]
            log.info(f"[A7670E] Trying AT port candidate: {at_candidate}")
            return at_candidate
    ...
    if usb_ports:
        log.info(f"[A7670E] Found USB serial ports: {usb_ports}")
        ...
        for port_path in usb_ports:
            try:
                ...
                test_ser.close()
                if "OK" in resp:
                    log.info(f"[A7670E] AT responded on {port_path}")
                    return port_path
    ...
    log.error("[A7670E] No USB serial ports found. Is the module plugged in?")
    return None
...
class A7670E:
    ...
    def __init__(self, port, baud, fallback_baud=9600, timeout=1.0,
        ...
        # Try primary baud
        log.info(f"[A7670E] Opening {port} at {baud} baud...")
        try:
            ...
            if self._probe():
                log.info(f"[A7670E] Connected at {baud} baud")
                self._connected = True
                return
        except serial.SerialException as e:
            log.error(f"[A7670E] Serial error at {baud}: {e}")

        # Fallback baud
        log.warning(f"[A7670E] No response at {baud}. Trying {fallback_baud}...")
        try:
            ...
            if self._probe():
                log.info(f"[A7670E] Connected at {fallback_baud} baud")
                self._baud = fallback_baud
                self._connected = True
                return
        except serial.SerialException as e:
            log.error(f"[A7670E] Serial error at {fallback_baud}: {e}")

        log.error("[A7670E] ERROR: No response at any baud rate.")
        log.error("[A7670E] Check wiring and power supply.")
        self._connected = False
    ...
    def close(self):
        ...
        if self.ser and self.ser.is_open:
            self.ser.close()
            log.info("[A7670E] Serial port closed")
    ...
    def _reopen_serial(self):
        ...
        try:
            ...
            time.sleep(0.5)
            self._flush()
            log.info("[A7670E] Serial port reopened")
        except serial.SerialException as e:
            log.error(f"[A7670E] Reopen error: {e}")
    ...
    def wake(self, max_attempts=10):
        ...
        for attempt in range(max_attempts):
            ...
            try:
                ...
                if "OK" in resp:
                    if attempt > 0:
                        log.info(f"[A7670E] Module awake (attempt {attempt + 1})")
                    return True
            except Exception:
                pass

            log.error(f"[A7670E] No response — waking "
            ...
            try:
                resp = self.send_command("AT", timeout=2.0)
                if "OK" in resp:
                    log.info("[A7670E] Awake after serial reopen")
                    self._connected = True
                    return True
            except Exception:
                pass
            ...
            if self._pwrkey_pin is not None:
                log.warning("[A7670E] Pulsing PWRKEY to restart module...")
                self._pwrkey_pulse(self._pwrkey_pin)
                time.sleep(3.0)
                try:
                    resp = self.send_command("AT", timeout=2.0)
                    if "OK" in resp:
                        log.info("[A7670E] Awake after PWRKEY pulse")
                        self._connected = True
                        return True
                except Exception:
                    pass
        ...
        log.error(f"[A7670E] FAILED to wake after {max_attempts} attempts")
        return False
    ...
    def _pwrkey_pulse(pin):
        ...
        """
        if GPIO is None:
            log.warning("[A7670E] WARNING: RPi.GPIO not available, skipping PWRKEY pulse")
            return
        log.info(f"[A7670E] PWRKEY pulse on GPIO {pin} ...")
        ...
        log.info("[A7670E] PWRKEY released — waiting 3 s for boot ...")
        time.sleep(3.0)
    ...
    def init_module(self):
        ...
        if not self._connected:
            log.error("[A7670E] Cannot init — not connected")
            return False

        log.info("[A7670E] Initializing module...")

        # Basic AT check
        resp = self.send_command("AT", timeout=2.0)
        if "OK" not in resp:
            log.error("[A7670E] ERROR: Module not responding")
            return False
        log.info("[A7670E] AT OK")

        # Disable echo for cleaner responses
        self.send_command("ATE0", timeout=1.0)
        log.info("[A7670E] Echo disabled")

        # Check SIM
        sim_status = self.check_sim()
        log.info(f"[A7670E] SIM: {sim_status}")
        if sim_status != "READY":
            log.error(f"[A7670E] WARNING: SIM not ready ({sim_status})")
            return False

        # Signal quality
        rssi = self.get_signal_quality()
        if rssi == 99:
            log.error("[A7670E] WARNING: No signal (RSSI=99)")
        elif rssi >= 20:
            log.info(f"[A7670E] Signal: {rssi}/31 (Excellent)")
        elif rssi >= 15:
            log.info(f"[A7670E] Signal: {rssi}/31 (Good)")
        elif rssi >= 10:
            log.warning(f"[A7670E] Signal: {rssi}/31 (Fair)")
        else:
            log.warning(f"[A7670E] Signal: {rssi}/31 (Weak)")

        # Network registration
        reg_status = self.check_registration()
        log.info(f"[A7670E] Network: {reg_status}")
        if reg_status not in ("home", "roaming"):
            log.warning("[A7670E] WARNING: Not registered on network")
        ...
        for line in resp.splitlines():
            if "+COPS:" in line:
                log.info(f"[A7670E] Operator: {line.strip()}")
                break

        # Set SMS text mode
        resp = self.send_command("AT+CMGF=1", timeout=2.0)
        if "OK" in resp:
            log.info("[A7670E] SMS text mode set")
        else:
            log.warning("[A7670E] WARNING: Could not set SMS text mode")

        log.info("[A7670E] Module initialized")
        return True
    ...
    def enable_gnss(self):
        ...
        success = "OK" in resp
        if success:
            log.info("[A7670E] GNSS engine powered on")
        else:
            log.error("[A7670E] WARNING: Failed to enable GNSS")
        return success
    ...
    def acquire_gps(self, timeout=30, poll_interval=5, progress_callback=None):
        ...
        """
        log.info(f"[A7670E] Searching for GPS satellites (timeout: {timeout}s)...")
        start = time.time()
        poll_count = 0
        ...
        while (time.time() - start) < timeout:
            ...
            if fix_status == 1 and lat is not None and lng is not None:
                ...
                log.info(f"  Lock acquired in {elapsed_final:.1f} seconds")
        ...
        log.error("[A7670E] GPS TIMEOUT — No fix obtained")
    ...
    def parse_gnss_response(self, response):
        ...
        for line in response.splitlines():
            ...
            try:
                ...
                log.error(f"  GNSS parse error: {e}")
                return (0, None, None, None)
        ...
        for attempt in range(retries):
            try:
                ...
                if not self.wake(max_attempts=3):
                    log.error(f"    Module unresponsive (attempt {attempt + 1}/{retries})")
                    continue
                ...
                if not self.wait_for_prompt(timeout=5.0):
                    log.error(f"    No '>' prompt (attempt {attempt + 1}/{retries})")
                ...
                log.error(f"    No confirmation (attempt {attempt + 1}/{retries})")

            except Exception as e:
                log.error(f"    SMS error: {e} (attempt {attempt + 1}/{retries})")
